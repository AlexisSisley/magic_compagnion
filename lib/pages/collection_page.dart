...
// VERSION CORRIGÉE (Layout sans AppBar)
...
class _CollectionPageState extends State<CollectionPage> with TickerProviderStateMixin {
  ...
  Future<void> _loadFullCardData() async {
    ...
    if (uniqueCardIdentifiers.isEmpty) {
      log("CollectionPage/Wishlist: Aucune donnée Scryfall à charger.");
      _fullCardData = [];
      return;
    }
    ...
    try {
      ...
      log('Erreur chargement données Scryfall (Collection/Wishlist): $e');
      if (mounted) {
  ...
  Future<void> _updateCollectionQuantity(DeckCard card, int change) async {
    await _collectionService.upsertCardInCollection(
      scryfallId: card.scryfallId, cardName: card.name, quantityToAdd: change,
    );
    await _loadData();
  }
  
  Future<void> _updateWishlistQuantity(DeckCard card, int change) async {
  ...
  Widget build(BuildContext context) {
    // --- MODIFICATION : Suppression de l'AppBar ---
    return Column(
      children: [
        // --- Titre de la page (remplace l'AppBar) ---
        Padding(
        ...
        // --- TabBar (remplace le 'bottom' de l'AppBar) ---
        TabBar(
          controller: _tabController,
          labelStyle: GoogleFonts.cinzel(fontWeight: FontWeight.bold),
          unselectedLabelStyle: GoogleFonts.cinzel(),
          tabs: [
            Tab(text: 'Ma Collection (${_getCardCount(_collection)})'),
            Tab(text: 'Wishlist (${_getCardCount(_wishlist)})'),
          ],
        ),
              ...
              : TabBarView(
                  ...
                  children: [
                    _buildGroupedCardListView(
                      cardList: _collection, 
                      onUpdateQuantity: _updateCollectionQuantity,
                      emptyMessage: "Votre collection est vide.\nAjoutez des cartes depuis la Recherche ou le Scanner."
                    ),
                    _buildGroupedCardListView(
                      cardList: _wishlist, 
                      onUpdateQuantity: _updateWishlistQuantity,
                      emptyMessage: "Votre liste de souhaits est vide.\nAjoutez des cartes depuis la Recherche."
                    ),
                  ],
                ),
        ),
  ...
  // _buildGroupedList, _getPrimaryType, _getCardCount, 
  // _buildGroupedCardListView, _buildManaCostRow, _getManaIcon
  // ...
  ...
  Widget _buildGroupedCardListView({
    required List<DeckCard> cardList,
    required Future<void> Function(DeckCard, int) onUpdateQuantity,
    ...
    if (cardList.isEmpty) {
      return Center(
        child: Padding(
          ...
          child: Text(emptyMessage, style: GoogleFonts.cinzel(color: Colors.white70, fontSize: 16), textAlign: TextAlign.center),
        ),
      );
    }
        ...
        return Column(
          ...
          children: [
            Padding(
              ...
              child: Text('${group.title} ($groupCardCount)', style: GoogleFonts.cinzel(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            ),
            Column(
              children: [
                    ...
                    child: ListTile(
                      onTap: () {
                        if (!scryfallCard.id.startsWith('LOCAL:')) {
                          Navigator.push(
                            context,
                            MaterialPageRoute(builder: (context) => RecognitionResultPage(cardName: scryfallCard.name)),
                          ).then((_) => _loadData());
                        }
                      },
                      title: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(card.name, style: GoogleFonts.cinzel(color: Colors.white, fontSize: 16)),
                          _buildManaCostRow(scryfallCard.manaCost),
                        ],
                      ),
                      leading: SizedBox(
                        width: 40,
                        child: Text('${card.quantity}x', style: GoogleFonts.cinzel(color: Colors.yellow.shade700, fontSize: 18, fontWeight: FontWeight.bold)),
                      ),
                      trailing: SizedBox(
                        width: 100,
                        child: Row(
                          children: [
                            IconButton(icon: const Icon(Icons.remove, color: Colors.white70), onPressed: () => onUpdateQuantity(card, -1)),
                            IconButton(icon: const Icon(Icons.add, color: Colors.white70), onPressed: () => onUpdateQuantity(card, 1)),
                          ],
                        ),
                      ),
                    ),
